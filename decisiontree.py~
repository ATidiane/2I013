...
def gen_features(state,id_team,id_player):
    ...
    #Le joueur de mon equipe le plus proche de la balle
    mpball = Mystate.proche_ball().distance(self.position_ball)
    #Le joueur de l'equipe adverse le plus proche de la balle
    apball= Mystate.proche_ball(2).distance(self.position_ball)
    #Difference entre la distance entre mon joueur et la balle et son joueur a la balle.
    p_ball = (mpball - apball)
    return [bpos.distance(mpos),bpos.distance(myg),bpos.distance(hisg), p_ball >= -0.2]
#Nom des features (optionel)
gen_features.names = ["ball_dist","mygoal_dist","hisgoal_dist", "plusproche_ball"]
